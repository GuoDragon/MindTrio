...
# 当前任务
1. 我把 transformers 版本改成了 4.55.0 ，详情要求见`https://gitee.com/mindspore-lab/mindnlp/blob/master/setup.py?utm_source=chatgpt.com`。不过可能还是有环境问题。
2. train.ipynb 会卡在 trainer.train() 很长时间最后报错，详情见`D:\lcl\MindSpore_CCNU_MindTrio\NewTest\train.ipynb`，里面有运行结果
3. 卡在 trainer.train() 的时候 npu 的占用情况为：
```
+------------------------------------------------------------------------------------------------+
...
+===========================+===============+====================================================+
```
1. 你可以参考我之前的代码`D:\lcl\MindSpore_CCNU_MindTrio\OldTest`，不过这是今年8月份的代码，所以可能会有一些差异